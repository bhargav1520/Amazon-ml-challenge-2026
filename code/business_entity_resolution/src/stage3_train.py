...
Loads preprocessed train cache and mined candidates, trains LightGBM,
calibrates optimal F_0.5 decision threshold, and saves the trained model.
"""
...
def run_stage3_train(
    ...
    """Trains the pairwise entity resolution model and calibrates threshold."""
    ...
    print("\n[2/4] Mining Training Pairs & Computing C++ String Features...")
    train_keys = list(train_candidates.keys())
    if len(train_keys) > max_train_entities:
        np.random.seed(42)
        sample_keys = set(np.random.choice(train_keys, size=max_train_entities, replace=False))
    else:
    ...
    # 3. Fit LightGBM
    print("\n[3/4] Fitting LightGBM Match Classifier...")
    ...
    print("\n[4/4] Calibrating Macro F_0.5 Decision Threshold...")
    ...
    print(f"\n🎯 [CALIBRATION RESULT] Optimal Threshold: {best_thresh:.3f} | Validation F_0.5 Score: {train_f05:.4f}")
    ...
    print(f"Saved Threshold: {thresh_save_path}")

    print("\n✅ [STAGE 3 COMPLETE] Model trained, evaluated, and saved successfully!")


def main():
    parser = argparse.ArgumentParser(description="Stage 3: Feature extraction and LightGBM training")
